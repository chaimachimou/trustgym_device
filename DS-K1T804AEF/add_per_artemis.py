...
def add_person():
    # Send a GET request to capture card information
    response = requests.get(capture_card_url, auth=HTTPDigestAuth(username, password))
    ...
    if response.status_code == 200:
        try:
            ...
            # Check if the response contains card information
            if "CardInfo" in data:
                card_info = data["CardInfo"]
                card_no = card_info.get("cardNo", "No card number available")
                card_type = card_info.get("cardType", "No card type available")

                # Print the card information (for debugging)
                print(f"Card Number: {card_no}")
                print(f"Card Type: {card_type}")

                # Proceed with adding a person if a card is captured
                body = {
                    ...
                    "cards": [{"cardNo": card_no}],  # Add the captured card number
                ...
                print("No card information found in the response.")
